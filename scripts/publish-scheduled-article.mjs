#!/usr/bin/env node
// Publishes one approved article from a Google Sheet CSV export.
// The sheet is the editorial source of truth; the repo tracks published articles by slug.
...
async function main() {
  ...
  if (!CSV_URL) {
    throw new Error("EDITORIAL_SHEET_CSV_URL is not set.");
  }

  const csv = CSV_URL.startsWith("file://")
